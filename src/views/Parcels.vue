...
<div>
    ...
    <div class="h-screen overflow-auto">
        ...
        <div v-for="parcel in orderedParcels_list">
            <RouterLink :to="{ name: 'package', params: { packageId: parcel.id } }">
                <div class="bg-gradient-to-r from-green to-light-green mx-6 py-2 px-4 mb-2">
                    <p class="text-white txet-lg">
                        <b>Sender:</b>{{ parcel.phone_number }}
                    <p class="text-white text-lg"><b>Inbox:</b>{{ parcel.title }}</p>
                    </p>
                </div>
            </RouterLink>
        </div>
        ...
        <div v-for="parcel in sendedParcels_list" class="h-32 overflow-auto">
            <RouterLink :to="{ name: 'package', params: { packageId: parcel.id } }">
                <div class="bg-gradient-to-r from-green to-light-green mx-6 py-2 px-4 mb-2">
                    ...
                    <p class="text-white text-lg"><b>Title:</b>{{ parcel.title }}</p>
                </div>
            </RouterLink>
        </div>
    </div>
